...
// --- Better Auth tables (camelCase columns as required by Kysely adapter) ---
// Check if schema is correct; if not (first run or old snake_case schema), recreate.
const userCols = (db.prepare("PRAGMA table_info('user')").all() as { name: string }[]).map(r => r.name);
const needsMigration = userCols.length === 0 || !userCols.includes("emailVerified") || !userCols.includes("banned");
